...
def render_diagnostics_panel(state: AppState) -> None:
    ...
    with ui.column().classes("w-full h-full overflow-y-auto p-6 gap-4"):
        ui.label("Diagnostics").classes("text-2xl font-semibold text-[#e6edf3]")
        ...
        with ui.card().classes(
            ...
            ui.label("Session").classes("text-sm font-semibold text-[#e6edf3] mb-3")
...
def _diag_row(label: str, value: str) -> None:
    with ui.row().classes(
        "w-full items-center justify-between py-1 border-b border-[#2b2f3d]/60"
    ):
        ui.label(label).classes("text-xs text-[#8b949e]")
        ui.label(value).classes("text-xs text-[#e6edf3] font-mono")
