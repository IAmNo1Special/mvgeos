...
def _render_approval_dialog(state: AppState, req: ApprovalRequest) -> None:
    ...
    with (
        ...
        ui.card().classes(
            "w-[560px] max-w-[92vw] bg-[var(--bg-overlay)] border "
            "border-[var(--bg-tint-deep)] "
            "rounded-xl p-5 gap-3"
        ),
    ):
        with ui.row().classes("w-full items-center justify-between"):
            with ui.row().classes("items-center gap-2"):
                ui.icon("shield", size="20px").classes("text-[var(--text-fuchsia)]")
                title = (
...
def _render_main_step(state: AppState, req: ApprovalRequest) -> None:
    ui.label(f'"{req.spell_name}" wants to cast:').classes(
        "text-sm text-[var(--text-secondary)]"
    )
    ui.label(describe_operation(req)).classes(
        "text-sm font-medium text-[var(--text-primary)]"
    )
    if not req.schema_validated:
        with ui.row().classes(
            "w-full items-center gap-2 bg-[var(--bg-danger-wash)] border "
            "border-[var(--text-danger-a40)] "
            "rounded-lg p-3"
        ):
            ui.icon("warning", size="16px").classes("text-[var(--text-danger)]")
            ui.label(
                "Unvalidated arguments: this spell has no parameter schema, "
                "so no persistent rule can cover it; every cast prompts."
            ).classes("text-xs text-[var(--text-danger-soft)]").mark(
                "approval_unvalidated_warning"
            )
    with ui.row().classes("gap-2 flex-wrap"):
        ui.label(f"Source: {spell_source_label(req)}").classes(
    ...
    if req.arguments:
        with ui.column().classes("w-full gap-0.5 bg-[var(--bg-dialog)] rounded-lg p-3"):
            for key, value in req.arguments.items():
                ui.label(f"{key}: {_format_arg_value(value)}").classes(
                    "text-xs font-mono text-[var(--text-violet-soft2)]"
                )
    with ui.row().classes("w-full gap-2"):
        ui.button(
            ...
            "flex-1 bg-[var(--accent-primary)] text-white"
        ).mark("approval_allow_once_btn")
        ui.button(
            "Deny once",
    ...
    with ui.row().classes("w-full justify-center"):
        ...
        ).mark("approval_manage_permissions_btn")
...
def _render_confirm_step(state: AppState, req: ApprovalRequest) -> None:
    ...
    if kind == "spell-allow":
        ...
        ui.label("Scope: global (all projects).").classes(
        ...
        if is_mutating_request(req):
            ...
            ).classes("text-xs text-[var(--text-danger)]")
        ...
        ).classes("text-base font-semibold text-[var(--text-primary)]")
        ui.label("Persistent for this project only.").classes(
            "text-xs text-[var(--text-secondary)]"
        )
        ui.label(
            "This is not a filesystem sandbox: a shell spell could still "
            "address paths outside the project."
        ).classes("text-xs text-[var(--text-warn-bright)]")
    else:
        ui.label("Unknown grant kind.").classes("text-xs text-[var(--text-danger)]")
    with ui.row().classes("w-full gap-2 justify-end"):
        ...
        ).props("unelevated").classes("bg-[var(--accent-primary)] text-white").mark(
...
def render_approval_badge(state: AppState) -> None:
    ...
    def approval_badge_view() -> None:
        ...
        with ui.row().classes(
            ...
            "bg-[var(--bg-pink-wash)] border-b "
            "border-[var(--text-fuchsia-a40)]"
        ):
            ui.icon("verified_user", size="14px").classes("text-[var(--text-fuchsia)]")
            ui.label(
                "Session approval active: mutating casts run without "
                "prompting until the session ends."
            ).classes("text-[11px] text-[var(--text-fuchsia-pale)] flex-1")
            ui.button(
                "Details",
                on_click=lambda: state.request_approval_permissions_open(),
            ).props("flat dense no-caps").classes(
                "text-[11px] text-[var(--text-fuchsia)]"
            ).mark("approval_badge_details_btn")
            ...
            ui.button("Revoke", on_click=_revoke_session).props(
                "flat dense no-caps"
            ).classes("text-[11px] text-[var(--text-danger)]").mark(
                "approval_badge_revoke_btn"
            )
...
def render_approval_permissions(state: AppState) -> None:
    ...
    ui.label("Approval permissions").classes(
        "text-sm font-semibold text-[var(--text-primary)]"
    )
    with ui.column().classes("w-full gap-3") as body:
        ui.spinner("dots", size="sm").classes(
            "self-center text-[var(--accent-primary)]"
        )
...
def _render_permissions_view(
    ...
    if view.warnings:
        with ui.column().classes(
            "w-full gap-1 bg-[var(--bg-danger-wash)] border "
            "border-[var(--text-danger-a40)] "
            "rounded-lg p-3"
        ):
            with ui.row().classes("items-center gap-1.5"):
                ui.icon("warning", size="14px").classes("text-[var(--text-danger)]")
                ui.label("Warnings").classes(
                    "text-xs font-semibold text-[var(--text-danger-soft)]"
                )
            for warning in view.warnings:
                ui.label(warning).classes("text-xs text-[var(--text-danger-soft)]")

    with ui.row().classes("w-full items-center gap-2"):
        ui.label("Session approve-all").classes(
        ...
        if view.session_approve_all:
            ...
            if view.session_id:
                ui.label(f"session {view.session_id}").classes(
                    "text-[11px] text-[var(--text-muted)] font-mono"
                )
            ui.button(
                "Revoke",
                on_click=lambda: _revoke("session", view.session_id or "session"),
            ).props("flat dense").classes("text-xs text-[var(--text-danger)]").mark(
                "approval_revoke_session"
            )
        else:
            ui.label("off").classes("text-xs text-[var(--text-muted)]")

    ui.separator().classes("bg-[var(--border-subtle)]")
    ui.label("Trusted projects").classes(
        "text-xs font-semibold text-[var(--text-primary)]"
    )
    if view.trusted_projects:
        for index, project in enumerate(view.trusted_projects):
            with ui.row().classes("w-full items-center gap-2"):
                ui.label(project.root).classes(
                    "text-xs font-mono text-[var(--text-violet-soft2)] flex-1 break-all"
                )
        ...
        ui.label("No trusted projects.").classes("text-xs text-[var(--text-muted)]")

    ui.separator().classes("bg-[var(--border-subtle)]")
    ui.label("Always allowed").classes(
        "text-xs font-semibold text-[var(--text-primary)]"
    )
    if view.always_allowed:
        ...
        ui.label("No always-allowed rules.").classes("text-xs text-[var(--text-muted)]")

    ui.separator().classes("bg-[var(--border-subtle)]")
    ui.label("Never allowed").classes(
        "text-xs font-semibold text-[var(--text-primary)]"
    )
    if view.never_allowed:
        ...
        ui.label("No never-allowed rules.").classes("text-xs text-[var(--text-muted)]")
    ...
    if view.recent_activity:
        ...
        with ui.column().classes(
            "w-full gap-1 max-h-40 overflow-y-auto bg-[var(--bg-dialog)] rounded-lg p-2"
        ):
            for record in view.recent_activity:
                ...
                ).classes("text-[11px] font-mono text-[var(--text-secondary)]")

    ui.separator().classes("bg-[var(--border-subtle)]")
    ui.label("Policy source").classes(
        "text-xs font-semibold text-[var(--text-primary)]"
    )
...
def _render_grant_card(
    ...
    with ui.column().classes(
        "w-full gap-1 bg-[var(--bg-dialog)] border border-[var(--border-subtle)] "
        "rounded-lg p-3"
    ):
        with ui.row().classes("w-full items-center gap-2"):
        ...
        ).classes("text-[11px] text-[var(--text-muted)]")
        ...
        for label in grant.constraint_labels:
            ui.label(label).classes("text-[11px] text-[var(--text-secondary)]")
