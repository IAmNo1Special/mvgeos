...
def render_status_bar(state: AppState) -> ui.row:
    ...
    bar = ui.row().classes(
        "w-full h-7 bg-[#0d0f14] border-t border-[#2b2f3d] items-center px-3 shrink-0"
    )
    ...
    with bar:
        ...
        # Mvge label
        ui.label("MvgeOS").classes("text-[10px] text-[#64748b] ml-2 mr-4")

        # Active model
        if state.selected_model:
            model_slug = state.selected_model.split("/")[-1].split(":")[0]
            ui.label(model_slug).classes("text-[10px] text-[#8b949e] font-mono mr-4")
        ...
        if state.is_channeling:
            ui.label("Channeling...").classes("text-[10px] text-[#3b82f6] italic mr-4")
