...
def render_shell(state: AppState) -> ui.row:
    ...
    shell_container = ui.row().classes(
        "w-screen h-screen overflow-hidden m-0 p-0 flex flex-row no-wrap bg-[#0e1117]"
    )
    ...
    with shell_container:
        ...
        with ui.column().classes(
            "flex-grow h-full flex flex-col justify-between "
            "overflow-hidden p-0 m-0 relative"
        ):
