...
def render_review_rail(state: AppState) -> ui.column:
    ...
    container = (
        ui.column()
        .classes(
            "h-full bg-[#13151b] border-l border-[#2b2f3d] shrink-0 "
    ...
    with container:
        ...
        with ui.row().classes(
            ...
            ui.label("Review").classes(
                "text-xs font-semibold text-[#e6edf3] uppercase tracking-wider"
            )
            ui.button(
                icon="close",
        ...
        # Permission mode
        with ui.row().classes("px-3 py-2 border-b border-[#2b2f3d]/60"):
            ui.label("Permission mode").classes(
                "text-[10px] font-medium text-[#64748b] uppercase tracking-wider"
            )
        ...
        # Changed files
        with ui.row().classes(
            "px-3 py-2 border-b border-[#2b2f3d]/60 items-center justify-between"
        ):
        ...
        with ui.column().classes("flex-1 overflow-y-auto px-3 py-2"):
            if not state.changed_files:
                ui.label("No changes detected").classes("text-[11px] text-[#64748b]")
            else:
                for cf in state.changed_files:
                    with (
                        ...
                        badge_color = {
                            ...
                            "deleted": "text-[#ef4444]",
                            "renamed": "text-[#3b82f6]",
                        }.get(cf.status, "text-[#8b949e]")
                        ui.label(cf.status.upper()[:3]).classes(
                            f"text-[10px] font-mono font-bold {badge_color}"
                        )
                        ui.label(cf.path).classes("text-[#e6edf3] truncate flex-1")
