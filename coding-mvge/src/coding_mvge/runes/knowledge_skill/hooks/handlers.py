...
class KnowledgeHooks:
    ...
    async def on_turn_end(self, data: TurnEndData) -> None:
        ...
        try:
            ...
            logger.warning("Knowledge consolidation failed on turn_end: %s", exc)

    async def on_session_shutdown(self, data: SessionShutdownData) -> None:
        await self.knowledge.flush()
        await self.harvester.persist_buffer()
        if self.raw_knowledge_dir:
            await self.harvester.persist_to_raw_knowledge(
